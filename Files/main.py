...
def process_subscription(subscription):
    ...
    for config in configs:
        # فرض می‌کنیم کانفیگ‌ها به فرمت استاندارد Vmess، Vless و غیره هستند
        # اینجا باید منطق تبدیل به پروکسی‌های YAML اضافه شود
        # این فقط یک نمونه ساده است
        proxies_list.append(f"  - {config}")  # نیاز به پردازش دقیق‌تر دارد
    
    yaml_content = yaml_content.replace("%%PROXIES_LIST%%", '\n'.join(proxies_list) if proxies_list else '[]')
